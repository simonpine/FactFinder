    ...
    return (
        <article className='NewCardCont'>
            <div className="firstRowNew">
                <img style={newDetails.polarization? {}: {width: '100%'}} alt={`News representation from ${newDetails.source_id}`} src={newDetails.image_url ? newDetails.image_url : errorImg} />
                {newDetails.polarization ?
                    <div className='BarsCont'>
                        <div>
                            <h5 className='ClasificationText1'>Polarization: </h5>
                            <h5 className='ClasificationText' style={{ color: (100 * Math.abs(newDetails.polarization)) > 70 ? '#ff2b2b' : (100 * Math.abs(newDetails.polarization)) > 30 ? '#e872ff' : '#24FF00' }}>{`${newDetails.polarization ? Math.round(100 * (Math.abs((newDetails.polarization)))) !== 0 ? Math.round(100 * (Math.abs((newDetails.polarization)))) : '<1' : 'Server Error'}${newDetails.polarization ? '%' : ''}`}</h5>
                        ...
                        <div>
                            ...
                            <h5 className='ClasificationText' style={{ color: (100 * Math.abs(newDetails.falsity)) > 70 ? '#ff2b2b' : (100 * Math.abs(newDetails.falsity)) > 30 ? '#e872ff' : '#24FF00' }}>{`${newDetails.falsity ? Math.round(100 * (Math.abs((newDetails.falsity)))) !== 0 ? Math.round(100 * (Math.abs((newDetails.falsity)))) : '<1' : 'Server Error'}${newDetails.falsity ? '%' : ''}`}</h5>
